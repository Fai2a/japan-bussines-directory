...
export function QASection({ businessId, businessName }: { businessId: number; businessName: string }) {
  ...
  return (
    <section>
      <h2 className="mb-2 font-display text-xl font-bold text-ink">{t('questionsAnswers')}</h2>
      <p className="text-sm text-ink-soft">{t('askIntro', { name: businessName })}</p>

      <form onSubmit={ask} className="mt-3 flex gap-2">
        <input
          value={draft}
          onChange={(e) => setDraft(e.target.value)}
          placeholder={session?.user ? t('askPlaceholder') : t('signInToAsk')}
          className="flex-1 rounded-md border border-rule bg-panel px-3 py-2 text-sm focus:outline-none focus-visible:border-indigo"
        />
        <button type="submit" disabled={busy} className="btn btn-secondary disabled:opacity-60">{busy ? t('asking') : t('ask')}</button>
      </form>
      ...
      <div className="mt-4 space-y-3">
        {questions === null ? (
          ...
          <p className="text-sm text-meta">{t('noQuestions')}</p>
        ) : (
    ...
    <div className="panel p-4">
      ...
      {q.answers.map((a) => (
        <div key={a.id} className={`mt-2 rounded-md p-3 text-sm ${a.isOwner ? 'border-l-2 border-indigo bg-indigo-wash/50' : 'bg-[#f4f3ee]'}`}>
          <p className="font-medium text-ink">{a.isOwner ? t('owner') : a.author}<span className="ml-2 text-2xs font-normal text-meta">{shortDate(a.createdAt)}</span></p>
          <p className="mt-0.5 text-ink-soft">{a.text}</p>
        </div>
      ))}
      {answering ? (
        <form onSubmit={submit} className="mt-2 flex gap-2">
          <input value={text} onChange={(e) => setText(e.target.value)} placeholder={t('yourAnswerPlaceholder')} autoFocus className="flex-1 rounded-md border border-rule bg-panel px-3 py-1.5 text-sm focus:outline-none focus-visible:border-indigo" />
          <button type="submit" disabled={busy} className="btn btn-secondary py-1.5 text-sm disabled:opacity-60">{busy ? t('posting') : t('post')}</button>
        </form>
      ) : (
        session?.user && <button onClick={() => setAnswering(true)} className="mt-2 text-xs font-semibold text-indigo hover:underline">{t('answerThis')}</button>
      )}
    </div>
  );
}
