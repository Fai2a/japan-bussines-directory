    ...
    <div className="shell py-8">
      <Breadcrumbs
        items={[
          ...
          { href: `/categories?group=${group.key}`, label: group.name },
          { label: cat.name },
        ]}
      />
      ...
      <header className="relative isolate mt-4 overflow-hidden rounded-lg border border-rule">
        ...
        <div className="p-6 sm:p-8">
          ...
          <h1 className="font-display text-3xl font-extrabold tracking-tight text-white sm:text-4xl">
            {cat.name} <span className="font-jp text-2xl font-medium text-white/75">{cat.nameJa}</span>
          </h1>
          <p className="mt-2 max-w-xl text-white/85">{cat.blurb}</p>
          <p className="tnum mt-3 text-sm text-white/70">{cat.count.toLocaleString('en-US')} listings nationwide</p>
        </div>
      </header>

      <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_260px]">
        <div>
          <FilterBar cities={citiesWithListings.map((c) => ({ slug: c.slug, name: c.name }))} total={sorted.length} />
          ...
          {total === 0 ? (
            <div className="panel mt-6 p-10 text-center">
              ...
              <p className="mt-1 text-ink-soft">Try removing a filter, or explore a nearby city.</p>
              <Link href={`/category/${cat.slug}`} className="btn btn-secondary mt-4">Clear filters</Link>
            </div>
          ) : (
            <ResultsView
              featured={featured}
        ...
        <aside className="space-y-6">
          ...
          <div className="panel p-4">
            <h2 className="eyebrow mb-3">Popular cities</h2>
            <ul className="space-y-1.5">
