...
export const metadata: Metadata = {
  title: 'Buzz — articles & guides',
  description: 'Guides on getting listed, writing useful reviews, verification, and using the Data Hub — plus perspective on local discovery in Japan.',
};
...
export default function BuzzPage() {
  ...
  return (
    <div className="shell py-8">
      <Breadcrumbs items={[{ href: '/', label: 'Home' }, { label: 'Buzz' }]} />
      <header className="mt-4">
        <p className="eyebrow">The NihonPages blog</p>
        <h1 className="mt-1 font-display text-4xl font-extrabold tracking-tight text-ink">Buzz</h1>
        <p className="mt-2 max-w-xl text-ink-soft">Guides for owners and visitors, product notes, and the occasional opinion about local discovery in Japan.</p>
      ...
      <Link href={`/buzz/${lead.slug}`} className="group mt-8 grid overflow-hidden rounded-lg border border-rule md:grid-cols-2">
        ...
        <div className="flex flex-col justify-center bg-panel p-6 sm:p-8">
          ...
          <p className="mt-3 text-xs text-meta">{lead.author} · {shortDate(lead.date)} · {lead.readMins} min read</p>
        </div>
      </Link>
        ...
        {rest.map((a) => (
          <Link key={a.slug} href={`/buzz/${a.slug}`} className="group flex flex-col overflow-hidden rounded-md border border-rule">
            ...
            <div className="flex flex-1 flex-col bg-panel p-4">
              ...
              <p className="mt-3 text-xs text-meta">{shortDate(a.date)} · {a.readMins} min</p>
            </div>
          </Link>
        ))}
      </div>
